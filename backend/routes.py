        ...
        for sid in site_ids:
            ...
            try:
                ...
                script = (
                    ...
                    f"const resp = await fetch(url.toString(), {{method: request.method, headers: hdrs, body: request.body, redirect: 'manual'}});"
                    f"if(resp.status>=300&&resp.status<400){{"
                    f"let loc=resp.headers.get('Location')||'';"
                    f"let hdrs2=new Headers();"
                    f"resp.headers.forEach((v,k)=>{{if(k.toLowerCase()!=='location')hdrs2.set(k,v);}});"
                    f"hdrs2.set('Location',loc.replace('https://{target_host}','https://{domain}').replace('http://{target_host}','https://{domain}'));"
                    f"return new Response(null,{{status:resp.status,headers:hdrs2}});"
                    f"}}"
                    ...
                    f"text=text.split('https://{target_host}').join('https://{domain}')"
                    f".split('http://{target_host}').join('https://{domain}')"
                    f".split('https:\\\\/\\\\/{target_host}').join('https:\\\\/\\\\/{domain}')"
                    f".split('http:\\\\/\\\\/{target_host}').join('https:\\\\/\\\\/{domain}');"
...
<p>If you have any questions about our Return Policy, please do not hesitate to contact our support team.</p>"""
    ...
    def woocommerce_products_list():
        """List WooCommerce products. Filter by site_id query param. Supports pagination."""
        try:
            site_id = request.args.get("site_id", type=int)
            page = request.args.get("page", type=int)
            limit = request.args.get("limit", type=int, default=50)
            if page is not None and page > 0:
                products, total = list_woocommerce_products(site_id, page=page, limit=limit)
                return jsonify({
                    "code": 200,
                    "data": {
                        "items": products,
