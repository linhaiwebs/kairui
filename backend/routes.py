        ...
        for sid in site_ids:
            ...
            try:
                ...
                script = (
                    ...
                    f"if(loc){{resp.headers.set('Location',loc.replace('https://{target_host}','https://{domain}').replace('http://{target_host}','https://{domain}'));}}"
                    f"return new Response(resp.body,{{status:resp.status,headers:resp.headers}});"
                    f"}}"
