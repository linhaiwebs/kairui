        ...
        for sid in site_ids:
            ...
            try:
                ...
                script = (
                    ...
                    f"function _rewrite(v){{if(!v)return v;if(v.indexOf('wp-json/kairui')>-1||v.indexOf('wp-admin')>-1||v.indexOf('wp-content/plugins')>-1)return v;v=v.replace('http://{target_host}','https://{domain}').replace('https://{target_host}','https://{domain}').replace('{target_host}','{domain}');return v;}}"
                    ...
                    f"if(ct.includes('text/html')){{"
                    f"return new HTMLRewriter().on('a[href]',{{element(el){{var h=el.getAttribute('href');if(h)el.setAttribute('href',_rewrite(h));}}}})"
                    f".on('img[src]',{{element(el){{var s=el.getAttribute('src');if(s)el.setAttribute('src',_rewrite(s));}}}})"
                    f".on('form[action]',{{element(el){{var a=el.getAttribute('action');if(a)el.setAttribute('action',_rewrite(a));}}}})"
                    f".on('link[href]',{{element(el){{var h=el.getAttribute('href');if(h)el.setAttribute('href',_rewrite(h));}}}})"
                    f".transform(newResp);"
                    f"}}else if(ct.includes('application/json')||ct.includes('text/plain')){{"
                    f"let text=await newResp.text();"
                    f"text=text.replace(new RegExp('https://{target_host}','g'),'https://{domain}').replace(new RegExp('http://{target_host}','g'),'https://{domain}');"
                    f"return new Response(text,{{status:newResp.status,headers:newResp.headers}});"
                    f"}}"
                    f"return newResp;"
                    "}"
                )
    ...
    def _regenerate_static_site_html(task_id_or_none, site_id):
        ...
        if static_dir:
            try:
                ...
                logger.warning(f"Regenerate: no SSH env for {domain}")
                return
            ...
            for rel_path, content in files.items():
                if rel_path.endswith(".css") or rel_path.endswith(".js"):
                    continue
                ssh.write_file(f"{static_dir}/{rel_path}", content)
                uploaded += 1
            ssh.reload_nginx()
...
"""
            ssh.write_file(nginx_conf, nginx_conf_content)
            ssh.reload_nginx()
        ...
        try:
            ...
            for dom in domains:
                ...
                site_data = {
                    ...
                    "nginx_alias": alias,
                    "created_by": user_id,
                    "panel_environment_id": panel_env.get("id") if panel_env else None,
                ...
                panel_port = 22  # SSH port
                panel_key = panel_env.get("ssh_password", "") if panel_env else ""
...
<p>If you have any questions about our Return Policy, please do not hesitate to contact our support team.</p>"""
    ...
    def amazon_google_hotness():
        ...
        try:
            ...
            # Build zero-value entries for keywords that returned no data
            for kw in keywords:
                if kw not in vol_data:
                    vol_data[kw] = {"search_volume": 0, "competition": 0, "cpc": 0}
